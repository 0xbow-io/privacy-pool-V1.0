  ...
  const AccSelectionCard = (className: string) => {
    return (
      <div
        id={'AccountCard'}
        className={cn(
          'relative flex flex-col justify-start gap-4 rounded-3xl px-4 py-4 duration-300 ease-in',
          className,
        )}
      >
        <div>
  ...
  const ExtValueContainer = (className?: string) => {
    return (
      <div
        id={'extValContainer'}
        className={cn(
          'relative grid h-full w-full grid-cols-1 grid-rows-4 items-start justify-between gap-y-2 duration-300 ease-in',
          className,
        )}
      >
        <div className="border-b-2 pb-2">
          <label className="text-lg font-normal">Compute Output </label>
        </div>
        <div className="place-self-end ">
          <h2 className="text-lg font-semibold"> {inputCommitment1Value.toString()} ETH </h2>
        </div>
        <div className="place-self-end ">
          <h2 className="text-lg font-semibold"> {inputCommitment1Value.toString()} ETH </h2>
        </div>
          ...
          <div className="flex flex-row items-center justify-center">
            ...
            <div>
              <Input
                ...
                onChange={(e) => handleOutCommitmentAmntChange(e)}
                className="bg-font-color   rounded-none	 border-none  px-0 text-end text-lg font-semibold text-page-background  placeholder:text-page-background focus:appearance-none"
              />
            </div>
          </div>
        </div>
  ...
  const TxRecordSection = (className: string) => {
    return (
      <div
        id="txRecordGrid"
        className={cn(
          ' text-font-color relative flex flex-col items-start justify-start gap-10 rounded-xl bg-page-background  px-10  duration-300 ease-in',
          currentPool === '' ? 'z-10 blur-2xl' : ' ',
          className,
        )}
      >
        <div className="border-border-color flex w-full border-b-4 py-10">
          <h1 className={cn('text-font-color text-5xl font-bold')}>
            Create {publicVal >= 0 ? 'Commit' : 'Release'}:
          </h1>
        </div>

        <div className=" relative grid grid-cols-3 grid-rows-4 items-start justify-between rounded-xl text-page-background">
          {InputCommitmentCard(
            '  px-4 py-4 border border-page-background border-r-0 rounded-tl-xl bg-font-color',
            1,
          )}
          {InputCommitmentCard(
            ' px-4 py-4 border border-page-background border-r-0 bg-font-color ',
            2,
          )}
          {InputSummaryCard(
            'col-start-3 h-full row-span-2  px-4 py-4  border border-page-background rounded-tr-xl bg-font-color ',
          )}
          {ExtValueContainer(
            'col-start-1 row-start-2 col-span-2 row-span-2 border-t-0 border border-page-background bg-font-color px-6 py-6',
          )}
        </div>
      </div>
    );
