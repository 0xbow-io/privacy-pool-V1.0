...
// from zk-kit: https://github.com/privacy-scaling-explorations/zk-kit/blob/main/packages/imt.sol/contracts/internal/InternalLeanIMT.sol
// Delegates hashing to external PoseidonT3 contract
...
struct LeanIMTData {
    ...
    // external PoseidonT3 contract for hashing
...
library InternalLeanIMT {
    ...
    function _insert(LeanIMTData storage self, uint256 leaf) internal returns (uint256) {
        ...
        for (uint256 level = 0; level < treeDepth;) {
            if ((index >> level) & 1 == 1) {
                node = self.PoseidonT3.hash([self.sideNodes[level], node]);
            } else {
    ...
    function _insertMany(LeanIMTData storage self, uint256[] memory leaves) internal returns (uint256) {
        ...
        for (uint256 level = 0; level < treeDepth;) {
            ...
            for (uint256 i = 0; i < numberOfNewNodes;) {
                ...
                if (rightNode != 0) {
                    parentNode = self.PoseidonT3.hash([leftNode, rightNode]);
                } else {
                    parentNode = leftNode;
                }
    ...
    function _update(LeanIMTData storage self, uint256 oldLeaf, uint256 newLeaf, uint256[] memory siblingNodes)
        internal
        returns (uint256)
    {
        ...
        for (uint256 level = 0; level < treeDepth;) {
            if ((index >> level) & 1 == 1) {
                ...
                node = self.PoseidonT3.hash([siblingNodes[i], node]);
                oldRoot = self.PoseidonT3.hash([siblingNodes[i], oldRoot]);
                ...
                if (index >> level != lastIndex >> level) {
                    ...
                    node = self.PoseidonT3.hash([node, siblingNodes[i]]);
                    oldRoot = self.PoseidonT3.hash([oldRoot, siblingNodes[i]]);
    ...
    function _remove(LeanIMTData storage self, uint256 oldLeaf, uint256[] memory siblingNodes)
        internal
        returns (uint256)
    {
