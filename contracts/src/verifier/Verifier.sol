...
contract Verifier is IVerifier, State {
    ...
    function _requireValidFees(
        IPrivacyPool.Request calldata _r,
        IPrivacyPool.GROTH16Proof calldata _proof
    ) internal pure {
        /// fee can't be greater than the total sum of IO values
        uint256 SumOfIO = _proof._pubSignals[D_ExternIO_StartIdx] +
    ...
    function _requireValidFeeCollector(
        IPrivacyPool.Request calldata _r
    ) internal pure {
    ...
    function _requireValidStateRootDepth(
        IPrivacyPool.GROTH16Proof calldata _proof
    ) internal view {
        /// extract the stateRoot & depth from the _proof public inputs
        uint256 _stateRootSignal = _proof._pubSignals[
            D_ExistingStateRoot_StartIdx
        ];
        uint256 _treeDepth = _proof._pubSignals[D_ActualTreeDepth_StartIdx];

        uint256 _actualDepth = FetchDepthForStateRoot(_stateRootSignal);
        if (_actualDepth == 0) {
    ...
    function _requireValidOutputs(
        ...
        for (
            uint8 i = D_MAX_ALLOWED_EXISTING;
            i < D_MAX_ALLOWED_EXISTING + D_MAX_ALLOWED_NEW;
            i++
        ) {
            if (_proof._pubSignals[D_NewNullRoot_StartIdx + i] != 0) {
                revert InvalidNullRoot();
