...
import {
    ...
    SNARK_SCALAR_FIELD,
    D_SCOPE_IDX,
    D_ACTUAL_TREE_DEPTH_IDX,
    D_EXTERN_IO_IDX_MIN,
    D_EXTERN_IO_IDX_MAX,
    D_EXISTING_STATE_ROOT_IDX,
    D_NEW_SALT_PUBLIC_KEY_IDX_MIN,
    D_NEW_SALT_PUBLIC_KEY_IDX_MAX,
    D_NEW_CIPHERTEXT_IDX_MIN,
    D_NEW_CIPHERTEXT_IDX_MAX,
    D_NEW_NULL_ROOT_IDX_MIN,
    D_NEW_NULL_ROOT_IDX_MAX,
    D_NEW_COMMITMENT_ROOT_IDX_MIN,
    D_NEW_COMMITMENT_ROOT_IDX_MAX,
    D_NEW_COMMITMENT_HASH_IDX_MIN,
    D_NEW_COMMITMENT_HASH_IDX_MAX,
    D_CONTEXT_IDX,
    D_MIN_MT_DEPTH,
    D_MAX_MT_DEPTH,
    D_CIPHERTEXT_SIZE,
...
contract Verifier is IVerifier, State {
        ...
        pure
    {
        ...
        uint256 SumOfIO = _proof._pubSignals[D_EXTERN_IO_IDX_MIN] + _proof._pubSignals[D_EXTERN_IO_IDX_MAX];
        if (_r.fee >= SumOfIO) {
            revert FeeTooHigh(_r.fee, SumOfIO);
        }
    }

    /// @dev modifier that only executes the function body only if the input value > 0
    ...
    /// @dev modifier that only executes the function body only if the specified exteranl output value > 0
    ...
    function _requireValidSrcAndSink(IPrivacyPool.Request calldata _r, IPrivacyPool.GROTH16Proof calldata _proof)
        ...
        pure
    {
        if (_fetchCommitmentAmt(_proof) > 0 && _r.sink == address(0)) {
            revert SrcIsZero();
        }
    ...
    function Scope() internal view returns (uint256) {
        return uint256(
            keccak256(
                abi.encode(
                    GetChainId(),
    ...
    function _requireValidScope(IPrivacyPool.GROTH16Proof calldata _proof) internal view {
        ...
        uint256 _scopeSignal = _proof._pubSignals[D_SCOPE_IDX];
    ...
    function _requireValidContext(IPrivacyPool.Request calldata _r, IPrivacyPool.GROTH16Proof calldata _proof)
        ...
        view
    {
        uint256 _contextSignal = _proof._pubSignals[D_CONTEXT_IDX];
        uint256 _actualContext = Context(_r);
        if (_contextSignal != _actualContext) {
            revert InvalidContext(_contextSignal, _actualContext);
        }
    }
    ...
    function _requireValidStateRootDepth(IPrivacyPool.GROTH16Proof calldata _proof) internal view {
        /// extract the stateRoot & depth from the _proof public inputs
        uint256 _stateRootSignal = _proof._pubSignals[D_EXISTING_STATE_ROOT_IDX];
        uint256 _treeDepth = _proof._pubSignals[D_ACTUAL_TREE_DEPTH_IDX];

        uint256 _actualDepth = FetchTreeDepthForRoot(_stateRootSignal);
        if (_actualDepth == 0) {
            revert InvalidStateRoot(_stateRootSignal, _treeDepth, _actualDepth);
        }
    }
    ...
    function _requireValidOutputs(IPrivacyPool.GROTH16Proof calldata _proof) internal pure {
        for (uint8 i = 0; i < D_MAX_ALLOWED_EXISTING; i++) {
            if (_proof._pubSignals[D_NEW_NULL_ROOT_IDX_MIN + i] == 0) {
                revert InvalidNullRoot();
            }
            if (_proof._pubSignals[D_NEW_COMMITMENT_ROOT_IDX_MIN + i] != 0) {
                revert InvalidCommitmentRoot();
            }
            if (_proof._pubSignals[D_NEW_COMMITMENT_HASH_IDX_MIN + i] != 0) {
