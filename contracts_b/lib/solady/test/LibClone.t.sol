...
contract LibCloneTest is SoladyTest, Clone {
    ...
    function testDeployERC1967(uint256 value_) public {
        ...
        assertEq(vm.load(clone, _ERC1967_IMPLEMENTATION_SLOT), bytes32(uint256(uint160(address(this)))));
    }
    ...
    function testDeployDeterministicERC1967I(uint256 value_, bytes32 salt) public {
        ...
        address predicted = LibClone.predictDeterministicAddressERC1967I(address(this), salt, address(this));
        assertEq(clone, predicted);
    ...
    function testCreateDeterministicERC1967(uint256 value_, bytes32 salt) public {
        if (saltIsUsed[salt]) {
            (bool deployed, address clone) = LibClone.createDeterministicERC1967(address(this), salt);
            assertEq(deployed, true);
            assertEq(clone, LibClone.predictDeterministicAddressERC1967(address(this), salt, address(this)));
            return;
        }
        ...
        assertEq(vm.load(clone_, _ERC1967_IMPLEMENTATION_SLOT), bytes32(uint256(uint160(address(this)))));
    }

    function testCreateDeterministicERC1967I(uint256 value_, bytes32 salt) public {
        if (saltIsUsed[salt]) {
            (bool deployed, address clone) = LibClone.createDeterministicERC1967I(address(this), salt);
            assertEq(deployed, true);
            assertEq(clone, LibClone.predictDeterministicAddressERC1967I(address(this), salt, address(this)));
            return;
        }
