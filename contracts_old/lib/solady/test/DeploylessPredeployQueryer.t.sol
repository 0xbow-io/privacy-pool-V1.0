        ...
        unchecked {
            ...
            if (_random() % 2 == 0) {
                for (uint256 i; i < t.n; ++i) {
                    t.seeds[i] = _random();
                    t.targetQueryCalldata[i] = abi.encodeWithSignature("generate(uint256)", t.seeds[i]);
                }
                ...
                for (uint256 i; i < t.n; ++i) {
                    assertEq(abi.decode(t.decoded[i], (bytes)), RandomBytesGeneratorLib.generate(t.seeds[i]));
                }
            }
            for (uint256 i; i < t.n; ++i) {
                t.seeds[i] = _random();
