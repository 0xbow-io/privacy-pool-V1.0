...
import { Hex, encodeAbiParameters, fromHex } from 'viem';
...
export class txRecord {
  ...
  public constructor(input: CTX[], output: CTX[]) {
  ...
  public hash(): bigint {
    return hash4([
      hash2([this.inputNullifiers[0], this.inputNullifiers[1]]),
      hash2([GetCommitment(this.outputCTXs[0]), GetCommitment(this.outputCTXs[1])]),
      this.publicVal,
  ...
  public GenProofInputs(
    ...
    this.inputCTXs.forEach((ctx, i) => {
      ...
      proofInputs.inSigR8.push([
        this.inputSigs[i].R8[0] as bigint,
        this.inputSigs[i].R8[1] as bigint,
      ]);
