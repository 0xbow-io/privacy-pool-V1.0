...
import {
  ...
  import { ProofInputs, genProofCallData, fullProve} from '@core/pool';
...
describe("Privacy Pool V1", function () {
    ...
    async function deployContracts() {
        ...
        const [owner, testAccount] = await hre.viem.getWalletClients();
    ...
    describe("Deployment", function () {
        test("Verifier should verify this proof", async () => {
            const { verifier } = await loadFixture(deployContracts);
            ...
            expect(await verifier.read.verifyProof(
                [
                    // _pA
                    [
        ...
        // To-DO Test Encrypt & Decrypt ciphertexts to recover Commitments

    describe("Testing Contract", function () {
        test("2 Virtual Inputs to 1 Virtual Output & 1 Real Output from empty Tree ", async () => {
            ...
            const { privacyPool, publicClient, testAccount } = await loadFixture(deployContracts);
            ...
            expect(extVal).toBe(120n)
            expect(expectedMerkleRoot).toBe(0n)

            const result = await fullProve(proofInputs, 'build/privacyPool/privacyPool_js/privacyPool.wasm',  'build/privacyPool/groth16_pkey.zkey') 
