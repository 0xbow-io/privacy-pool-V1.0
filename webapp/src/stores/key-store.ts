...
return createStore<KeyStore>()((set, get) => ({
        ...
        generate: (): PrivacyKey => {
            ...
            if (get().avilCommits.length == 0) {
                set((state) => ({
                    avilCommits: [
                        CreateCommitment(key, {amount: 0n}),
                        CreateCommitment(key, {amount: 0n})
                    ]
                }));
            }
        ...
        importFromJSON: (data: string) => {
            ...
            const _new_keys = jsonObj.keys.map((k: any) =>  CreatePrivacyKey(k.privateKey));
            ...
            set((state) => ({
                ...
                avilCommits: [
                    CreateCommitment(_new_keys[0], {amount: 0n}),
                    CreateCommitment(_new_keys[0], {amount: 0n})
                ], 
            }));
          },
        ...
        updateInCommit: (index: number, value: string) => {
            ...
            const commit = get().avilCommits.find((c) => c.hash.toString(16) === value);
            if (commit === undefined) {
        ...
        getAvailableInputOptions: (index: number) =>  {  
            ...
            const avail = get().avilCommits.filter((c) => c.hash.toString(16) !== _input_taken);
        ...
        getInTotalValueFormatted: (): number => {
            ...
            return Number(formatUnits(BigInt(get().inTotalValue), Number(unitRep.decimals)))
        },
        ...
        udpatePublicValue: (value: number): void => {
            ...
            set((state) => ({
                ...
                extraAmountReason: _expected_output < 0 ? 'total output value is negative' : '',
                publicValue: value,
        ...
        udpateOutputValue: (index: number, value: number): void => {
            ...
            const _expected_output = get().getInTotalValueFormatted() + get().publicValue;
            ...
            if (_total_output < 0) { 
                set((state) => ({
                    outputAmountIsValid: [false, false],
                    outputAmountReasons: ['total output value is negative', 'total output value is negative']
                }));
            }  else {

                // rebalance the output values based on the output
                if (_total_output != _expected_output ) {  // if less 
                ...
                if ( _total_output > _expected_output) {
                    set((state) => ({
                        outputAmountIsValid: [false, false],
                        outputAmountReasons: ['total output is more than expected, adjust external amount or lower output amount', 'total output is more than expected, adjust external amount or lower output amount']
                    }));
                } else {
                    set((state) => ({
        ...
        updateOutputPrivacyKey: (index: number, pubKeySerialized: string): void => {
            ...
            const key = get().keys.find((pk) => pk.pubKey.serialize() === pubKeySerialized);
            if (key === undefined) {
        ...
        getOutputPubKeyHash: (index: number): string => {
            ...
            return pK.pubKey.serialize() //pK.pubKey.serialize().substring(0,24)+'....'+ pK.pubKey.serialize().substring(60);
        },
        ...
        isInputValid: (): {ok: boolean, reason: string} => {
            ...
            get().inCommits.forEach((hash, i) => {
                ...
                const commit = get().avilCommits.find((c) => c.hash.toString(16) === hash);
                if (commit === undefined) {
                    _all_inputs_valid[i] = false;
                }
            });
            ...
            if (!_all_inputs_valid[0] || !_all_inputs_valid[1]) {
                return {ok: false, reason: 'input commitments must be set and available'};
            }
        ...
        isOutputValid: (): {ok: boolean, reason: string} => {
            ...
            if (_total_output != _expected_output) {
                return {ok: false, reason: 'total output values does not match expected'};
            }
